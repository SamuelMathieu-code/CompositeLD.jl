...
function getStrongLD(ref_genotypes::SnpData, 
    ...
    for (i, j) in enumerate(kept_indx)
        ...
        corr = [ld_r²(s, kept_indx[i], ref_genotypes.snparray) for s in indexes]
        kept = corr .> r2_tresh
        res[i] = indexes[kept]
    end
...
function getStrongLD(ref_genotypes::SnpData, 
    ...
    res = Vector{Vector{Int}}(undef, length(kept_indx))
    for (i, j) in enumerate(kept_indx)
        chr, pos = ref_genotypes.snp_info.chromosome[j], ref_genotypes.snp_info.position[j]
        idx_v_b = ref_genotypes.snp_info.chromosome .== chr .& abs.(ref_genotypes.snp_info.position .- pos) .< window
        indexes = ref_genotypes.snp_info.idx[idx_v_b]
        corr = [ld_r²(s, kept_indx[i], ref_genotypes.snparray) for s in indexes]
        kept = corr .> r2_tresh
        res[i] = indexes[kept]
...
function getStrongLD(ref_genotypes::AbstractVector{SnpData}, 
    ...
    snps_chr = Vector{Int}(undef, size(snps, 1))
    @threads for (i, chr_pos_sing) in collect(enumerate(snps))
        local j = searchsortedfirst(ref_genotypes[chr_pos_sing[1]].snp_info.snpid, chr_pos_sing)
        if j > lastindex(ref_genotypes[chr_pos_sing[1]].snp_info.snpid) || ref_genotypes[chr_pos_sing[1]].snp_info.snpid[j] != chr_pos_sing
            j = -1
        end
        snps_indx[i] = j
        snps_chr[i] = snps[i][1]
    ...
    res_chrpos = Vector{Vector{Tuple{Int8, Int32}}}(undef, length(kept_indx))
    @threads for i in eachindex(kept_indx, kept_chr, res)
        pos = ref_genotypes[kept_chr[i]].snp_info.position[kept_indx[i]]
        idx_v_b = abs.(ref_genotypes[kept_chr[i]].snp_info.position .- pos) .< window
        indexes = ref_genotypes[kept_chr[i]].snp_info.idx[idx_v_b]
        corr = [ld_r²(s, kept_indx[i], ref_genotypes[kept_chr[i]].snparray) for s in indexes]
        kept = corr .> r2_tresh
        res[i] = indexes[kept]
